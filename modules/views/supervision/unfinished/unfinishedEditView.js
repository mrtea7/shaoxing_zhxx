...
var unfinishedEditView = BaseView.extend(module, {
    ...
    renderView: function () {
        ...
        this.renderItem(form, "attach", "附件", new UIButton(this, {label: '点击上传', style: 'ui-btn-success'}));
        ...
        var company = new UIGroup(this, {gap: 10, tag: "p"});
        for (var i = 0; i < companyList.length; i++) {
            var office = new UIHGroup(this, {gap: 10, tag: "p"});
            for (var j = 0; j < officeList.length; j++) {
                office.append(new UICheckbox(this, {label: officeList[j].bean.officeName, value: "3"}))
            }
            company.append(this.renderItem(form, companyList[i].bean.id, companyList[i].bean.name + " ->", office));



        }
        this.renderItem(form, "office", "科室", company);
...
unfinishedEditView.import(__basedir + "/framework/components/form/FileUploader.js");